...
fun main() {
     ...
     * 첫번쨰 요소부터 init함수를 호출하여 반환갑사으로 초기값 설정
     */
    val num = Array<Int>(10){i -> i}//10개의 정수를 저장하는 배열을 생성하고 0~9로 초기화
    //==val num = Array(10, {i->i})
    //== val num = Array<Int>(10) {it}//인자가 하나라면 it으로 받을 수 있따.
    for (value in num){
        print("$value, ")
    }
    println()
    ...
    val num3 = Array(10){_ -> 0} //매개 변수 'it'은 사용됮 않는 경우 _로 바꿀 수 있음
    ...
    println("num4: ${num4.contentToString()}")//num4 배열 값들을 contentToString()으로 출력
    ...
    for(x in array2D.indices){//행(0,1), indices 프로퍼티: 행의 인덱스 범윌를 반환(0..1)
